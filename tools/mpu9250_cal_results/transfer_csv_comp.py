...
ax[0].legend()
ax[0].grid()
ax[1].errorbar(PTB['freqs'],((PTB['phase']-PTBPhasemean)+np.pi)/np.pi*180,yerr=PTB['phaseuncer']/np.pi*180,label='PTB', fmt='o',markersize=MARKER_SIZE)
ax[1].errorbar(CEM['freqs'],(CEM['phase']-CEMPhasemean)/np.pi*180,yerr=CEM['phaseuncer']/np.pi*180,label='CEM', fmt='o',markersize=MARKER_SIZE)
